...
def vlans(self):
    ...
    tagged_vlan_ids = device.vc_interfaces.values_list('untagged_vlan', flat=True)
    untagged_vlan_ids = device.vc_interfaces.values_list('tagged_vlans', flat=True)
...
def vrfs(self):
    ...
    vrf_ids = device.vc_interfaces.values_list('ip_addresses__vrf', flat=True)
